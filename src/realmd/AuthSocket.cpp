...
bool AuthSocket::_HandleLogonProof()
{
    ...
    else
    {
        ...
        uint32 MaxWrongPassCount = sFileConfig.GetIntDefault("WrongPass.MaxCount", 0);
        if (MaxWrongPassCount > 0)
        {
            ...
            if (QueryResult* loginfail = LoginDatabase.PQuery("SELECT id, failed_logins FROM account WHERE username = '%s'", _safelogin.c_str()))
            {
                ...
                {
                    uint32 WrongPassBanTime = sFileConfig.GetIntDefault("WrongPass.BanTime", 600);
                    bool WrongPassBanType = sFileConfig.GetBoolDefault("WrongPass.BanType", false);
