...
int32 Player::getMaxTimer(MirrorTimerType timer)
{
    ...
    {
        case FATIGUE_TIMER:
            if (GetSession()->GetSecurity() >= sWorld.getConfig(CONFIG_UINT32_TIMERBAR_FATIGUE_GMLEVEL))
        ...
        case BREATH_TIMER:
        {
            if (!isAlive() || HasAuraType(SPELL_AURA_WATER_BREATHING) ||
                    GetSession()->GetSecurity() >= sWorld.getConfig(CONFIG_UINT32_TIMERBAR_BREATH_GMLEVEL))
                return DISABLED_MIRROR_TIMER;
        ...
        case FIRE_TIMER:
        {
            if (!isAlive() || GetSession()->GetSecurity() >= sWorld.getConfig(CONFIG_UINT32_TIMERBAR_FIRE_GMLEVEL))
                return DISABLED_MIRROR_TIMER;
