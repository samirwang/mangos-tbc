...
void Player::Update(uint32 update_diff, uint32 p_time)
{
    ...
    if (m_nextSave > 0)
    {
        if (update_diff >= m_nextSave)
        {
            ...
            SaveToDB();
            DEBUG_FILTER_LOG(LOG_FILTER_PLAYER_STATS, "Player '%s' (GUID: %u) saved", GetName(), GetGUIDLow());
        }
        else
...
void Player::AddQuest(Quest const* pQuest, Object* questGiver)
{
    ...
    if (questGiver)
    {
        ...
        {
            case TYPEID_UNIT:
                sCPlusMgr.QuestAccept(this, (Creature*)questGiver, pQuest);
                break;
            case TYPEID_ITEM:
            case TYPEID_CONTAINER:
                sCPlusMgr.QuestAccept(this, (Item*)questGiver, pQuest);
                break;
            case TYPEID_GAMEOBJECT:
                sCPlusMgr.QuestAccept(this, (GameObject*)questGiver, pQuest);
                break;
        }
...
void Player::RewardQuest(Quest const* pQuest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    {
        case TYPEID_UNIT:
            handled = sCPlusMgr.QuestRewarded(this, (Creature*)questGiver, pQuest);
            break;
        case TYPEID_GAMEOBJECT:
            handled = sCPlusMgr.QuestRewarded(this, (GameObject*)questGiver, pQuest);
            break;
    }
...
void Player::LeaveBattleground(bool teleportToEntryPoint)
{
    if (BattleGround* bg = GetBattleGround())
    {
        ...
        if (bg->isBattleGround() && !isGameMaster() && sWorld.getConfig(CONFIG_BOOL_BATTLEGROUND_CAST_DESERTER) && !sBattleGroundMgr.isTesting())
        {
