...
void GossipMenu::AddMenuItem(uint8 Icon, char const* Message, GossipActionList dtActionList, char const* BoxMessage, uint32 BoxMoney, bool Coded)
{
    MANGOS_ASSERT(m_gItems.size() <= GOSSIP_MAX_MENU_ITEMS);
    ...
    gItem.m_gSender = 0;
    gItem.m_gOptionId = 0;
    gItem.m_gOptionIds = dtActionList;
    gItem.m_gBoxMessage = BoxMessage;
    gItem.m_gBoxMoney = BoxMoney;
...
GossipActionList GossipMenu::MenuItemActionList(unsigned int ItemId)
{
    if (ItemId >= m_gItems.size())
        return {};

    return m_gItems[ItemId].m_gOptionIds;
}
...
GossipActionList PlayerMenu::GossipOptionActionList(unsigned int Selection)
{
    return mGossipMenu.MenuItemActionList(Selection);
}
